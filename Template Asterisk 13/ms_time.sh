...
LOG=$(for i in $(sudo /usr/sbin/asterisk -rx "sip show peers" | grep OK | grep -oP '\(\K[^\)]+' | sed 's/ms//g' | sort -n | awk '$1>199'); do /usr/sbin/asterisk -rx "sip show peers" | grep OK | grep $i; done)
